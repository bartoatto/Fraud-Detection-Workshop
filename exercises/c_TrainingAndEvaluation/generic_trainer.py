...
# DFS Projects have no separate artifacts store - they're just files in the working dir.
if is_git_based_project == "true":
    domino_artifact_dir = os.environ.get("DOMINO_ARTIFACTS_DIR", "/mnt/artifacts")
else:
    domino_artifact_dir = f"{os.environ.get('DOMINO_WORKING_DIR', '/mnt')}/artifacts"
    os.makedirs(domino_artifact_dir, exist_ok=True)
...
def add_plots_to_training(model, name, X_train, X_val, y_train, y_val, features, df, proba, domino_artifact_dir):
...
def train_and_log(
    ...
    with mlflow.start_run(run_name=name) as run:
        ...
        # -------- Plots (unchanged) --------
        add_plots_to_training(model, name, X_train, X_val, y_train, y_val, features, df, proba, domino_artifact_dir)
...
def train_fraud(model_obj, model_name, transformed_df_filename, random_state=None):
    ...
    model_artifact_dir = os.path.join(domino_artifact_dir, "c_training", model_name.lower().replace(' ', '_'))
    os.makedirs(model_artifact_dir, exist_ok=True)

    data_summary_path = Path(model_artifact_dir) / "data_summary.json"
